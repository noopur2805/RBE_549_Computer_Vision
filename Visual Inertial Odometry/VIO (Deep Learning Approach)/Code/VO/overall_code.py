...
def read_images_gt_function():
    ...
    return stacked_images_batch,stacked_pos,stacked_quat
...
def loss_fn( pos1, pos2, q1, q2):
    """
    Computes the mean squared error between the predicted relative position and quaternion
    difference and the ground truth values from the CSV file.
    
    
    """
    # Compute the ground truth relative position and quaternion difference
    true_relpos = relative_position(pos1, pos2)
    true_qdiff = quaternion_difference(q1, q2)
    
    # Compute the mean squared error between the predicted and ground truth values
    K = 100
    pred_relpos, pred_qdiff = output
    mse_relpos = tf.reduce_mean(tf.square(pred_relpos - true_relpos))
    mse_qdiff = tf.reduce_mean(tf.square(pred_qdiff - true_qdiff))
    mse = mse_relpos + K * mse_qdiff
    return mse


@tf.function
def grad_fn(model, inputs, pos,q):
    with tf.GradientTape() as tape:
        output_pos, output_orientation = model(inputs)
    
        loss = loss_fn(output_pos, output_orientation, pos, q)
...
model_checkpoint = ModelCheckpoint('model_VO_checkpoint.hdf5', monitor='train_loss', verbose=1)



# Train the model
history = model.fit([np.array(stacked_images_batch)],[stacked_pos,stacked_quat], epochs=100, batch_size=32, verbose=1, callbacks=[model_checkpoint])
...
model.save('trained_VO_model.h5')