...
class MSCKF(object):
    ...
    def initialize_gravity_and_bias(self):
        ...
        gravity_imu = sum_linear_acc /size  #This is the gravity in the IMU frame.
    ...
    def batch_imu_processing(self, time_bound):
        ...
        used_imu_msg_cntr = 0 # Counter how many IMU msgs in the buffer are used 
        ...
        for imu_msg in self.imu_msg_buffer:
            ...
            gyro_m = imu_msg.angular_velocity
            acc_m = imu_msg.linear_acceleration 

            self.process_model(imu_time,gyro_m,acc_m)
            used_imu_msg_cntr +=1 
    ...
    def process_model(self, time, gyro_m, acc_m):
        """
        IMPLEMENT THIS!!!!!
        """
        """
        ...
        gyro = gyro_m- imu_state.gyro_bias
        acc = acc_m- imu_state.acc_bias 
        ...
        F[6:9,0:3] = -np.transpose(to_rotation(imu_state.orientation)).dot(skew(acc))
        ...
        Rkk1 = to_rotation(imu_state.orientation_null)
        # print("IMU NULL", imu_state.orientation_null)
        Phi[0:3,0:3] = to_rotation(imu_state.orientation).dot(np.transpose(Rkk1))
       
        u = Rkk1.dot(IMUState.gravity)
        ...
        Phi[6:9,0:3] = A1-(A1.dot(u)-w1)*s

        A2 = Phi[12:15,0:3]
        w2 = skew(dtime*imu_state.velocity_null+imu_state.position_null-imu_state.position).dot(IMUState.gravity)
        Phi[12:15,0:3] = A2- (A2.dot(u)-w2)*s

        # Propagate the state covariance matrix.

        Q = Phi.dot(G).dot(self.state_server.continuous_noise_cov).dot(np.transpose(G)).dot(np.transpose(Phi))*dtime
        self.state_server.state_cov[0:21,0:21] = Phi.dot(self.state_server.state_cov[0:21,0:21]).dot(np.transpose(Phi))+Q

        if len(self.state_server.cam_states)>0:
            self.state_server.state_cov[:21,21:] = Phi.dot(self.state_server.state_cov[0:21,21:])
            self.state_server.state_cov[21:,:21] = np.dot(self.state_server.state_cov[21:,:21] ,np.transpose(Phi))
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        if gyro_norm > 1e-5:
            dq_dt = (np.cos(gyro_norm*0.5)*np.eye(4) + np.sin(gyro_norm*dt*0.5)/gyro_norm * Omega).dot(q) 
            dq_dt2 =(np.cos(gyro_norm*0.25)*np.eye(4) + np.sin(gyro_norm*dt*0.25)/gyro_norm*Omega).dot(q) 
        else:
            dq_dt = np.cos(gyro_norm*dt*0.5) * (np.eye(4) + Omega*dt*0.5).dot(q)
            dq_dt2 = np.cos(gyro_norm*dt*0.25) * (np.identity(4) + Omega*dt*0.25).dot(q)
    ...
    def measurement_update(self, H, r):
        ...
        for i,(_,cam_state_iter) in enumerate(self.state_server.cam_states.items()): 
